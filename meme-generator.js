...
var BingoGenerator = function (memeBingoList, options) {
    if (!options) {
        options = {};
    }
    ...
    if (memeBingoList.info && memeBingoList.info.combined === 'true') {
        if (memeBingoList[this.mode]) {
            memeBingoList = memeBingoList[this.mode];
        } else if (memeBingoList["normal"]) {
            memeBingoList = memeBingoList["normal"];
        } else {
            console.log("bingoList doesn't contain a valid sub goal list for mode: \"" + this.mode + "\"");
        }
    }

    this.goalsByDifficulty = memeBingoList;
    this.rowtypeTimeSave = memeBingoList.rowtypes;
    this.synergyFilters = memeBingoList.synfilters || {};

    // assemble a list of all goals sorted by the goals' times
    this.goalsList = [];
    for (var i = 1; i <= 25; i++) {
        this.goalsList = this.goalsList.concat(memeBingoList[i]);
    }
...
ootBingoGenerator = function (memeBingoList, opts) {

    var bingoGenerator = new BingoGenerator(memeBingoList, opts);
